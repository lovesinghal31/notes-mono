...
export default function NoteEditForm({ note }: { note: INote }) {
  ...
  const onSubmit = async (data: EditNoteSchemaType) => {
    try {
      setSaving(true)

      const response = await api.put<ApiResponse<INote>>(
        `/notes/${data.id}`,
        data
      )
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl px-6 py-6">
        ...
        <form id="edit-note-form" onSubmit={form.handleSubmit(onSubmit)}>
          <FieldGroup>
            {/* Title */}
            <Controller
              name="title"
